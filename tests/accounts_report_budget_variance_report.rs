use tokio_erp::erpnext::accounts::report::budget_variance_report::budget_variance_report::{
    build_budget_map, build_comparison_chart_data, build_report_data, get_budget_dimensions_query,
    get_budget_records_query, get_columns, BudgetDistribution, BudgetRecord, BudgetVarianceFilters,
    FiscalYear, GlEntry, ReportColumn,
};
...
fn budget_variance_aggregates_budget_actual_variance_and_chart_data() {
    ...
    let budget_map = build_budget_map(&budget_records, &actuals);
...
fn budget_variance_cumulative_is_disabled_for_yearly() {
    ...
    let budget_map = build_budget_map(&budget_records, &[]);
