...
| `financial_report_template` | 8 | 6 | 1 | 1 | ported | Rust preserves financial report template registry names, modules, report types, row counts, all 29 account category seed rows, and engine data models/filter validation helpers in `accounts_financial_report_template_registry` and `accounts_financial_report_engine`. Template JSON plus DB/query/formatter/chart engine side effects remain external. |
