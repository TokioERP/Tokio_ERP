...
| `payment_request` | 7 | 4 | 1 | 2 | ported | Rust covers allowed doctypes, metadata surface, payment reference validation, request amount JSON summing, submit status/outstanding planning, core get_amount branches, Payment Entry Reference status/outstanding updates, schedule reference merge, print-format list, dummy message template, validate-payment guard, payment order mapping, and open-payment-request query planning in `accounts_payment_request`. Remaining gateway/Frappe DB orchestration is not yet parity-tested. |
