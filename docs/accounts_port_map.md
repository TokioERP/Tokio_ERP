...
| `pos_invoice_merge_log` | 5 | 3 | 1 | 1 | ported | Rust covers metadata, duplicate/customer/status validations, serial-return split grouping, and controller hooks in `accounts_pos_invoice_merge_log`. Submit/cancel merge side-effect planning, tax/payment aggregation, enqueue/cancel helpers, and DB-backed tests remain to reach `parity_tested`. |
