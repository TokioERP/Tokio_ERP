...
| `financial_report_template` | 8 | 6 | 1 | 1 | parity_tested | Rust preserves financial report template registry names, modules, report types, row counts, and all 29 account category seed rows from `account_categories.json` in `accounts_financial_report_template_registry`. Template JSON files remain external metadata. |
