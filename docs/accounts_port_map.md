...
| `budget` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata, fiscal-year/company and duplicate/account validations, applicable flag/nulling behavior, existing-expense guard, budget distribution generation/recalculation/total validation, budget record orchestration, annual/monthly action selection, expense comparison messages/breakup, MR/PO/requested/ordered/actual query planning, item default fallback, accumulated monthly budget helper, and revise-budget planning in `accounts_budget`. JSON/JS UI metadata remains external. |
