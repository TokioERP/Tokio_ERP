...
| `journal_entry` | 8 | 3 | 2 | 2 | ported | Rust covers metadata, controller hooks, opening/clearance defaults, party validation, advance direction guards, multi-currency/exchange-rate handling, company-currency amount calculation, debit/credit totals, reference validation for SO/PO/SI/PI, against-account assignment, remarks/print totals, get-balance balancing row, GL map rows, payment-entry draft helpers, outstanding/account-detail helpers, and reverse/inter-company journal-entry draft mapping in `accounts_journal_entry`. Remaining lifecycle side effects, asset/depreciation, invoice discounting, reposting, inter-company persistence, and live Frappe DB query branches still need parity closure. |
