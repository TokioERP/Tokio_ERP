...
| `pos_invoice` | 7 | 4 | 1 | 2 | ported | Rust covers metadata, basic POS/customer validation, mode-of-payment guard, payment amount sign rules, change amount/account validation, POS profile company guard, outstanding amount, status transitions, before-submit/before-cancel/on-cancel planning, phone payment completion check, stock availability helpers, product-bundle availability, reserved quantity aggregation, stock insufficiency messages, serial/batch required-field guards, return item quantity/serial guards, POS profile missing-value planning, reset mode-of-payment planning, phone payment request payloads, update-payments totals, sales-return mapping, merge-log planning, item query filtering handoff, Sales Invoice payment row creation, unallocated payment cleanup, return Sales Invoice creation/consolidation planning, and serial/batch bundle delink-submit planning in `accounts_pos_invoice`. Remaining POS field default population, loyalty/on-submit side-effect orchestration, and live Frappe DB writes are not yet parity-tested. |
