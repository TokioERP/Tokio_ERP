...
| `payment_entry` | 6 | 3 | 1 | 2 | ported | Rust covers metadata, controller hooks, party account setup, internal-transfer clearing, payment type/mandatory/duplicate reference/bank reference guards, receive negative-outstanding guard, allocated amount guard for non-customer/supplier parties, received/base amount calculation, allocated/unallocated/difference amount calculation, exchange gain/loss deduction row maintenance, title/remarks/status/in-words helpers, transaction currency/rate selection including same-currency defaults, advance-reference gain/loss bypass, and party/bank/deduction GL map planning in `accounts_payment_entry`. Remaining reference-doc validation, latest outstanding/payment-term allocation, taxes, payment requests, payment schedule, blocked supplier, exchange-rate DB lookup for foreign currencies, advance-as-liability, reconciliation, exchange gain/loss journal, and live Frappe DB query branches still need parity closure. |
