...
| `payment_request` | 7 | 4 | 1 | 2 | ported | Rust covers allowed doctypes, metadata surface, payment reference validation, payment-request amount/currency/subscription validation, request amount JSON summing, submit status/outstanding planning, phone payment payloads, payment URL/email plans, core get_amount branches, create-payment-entry planning, Payment Entry Reference payment-request allocation/splitting, Payment Entry Reference status/outstanding updates, schedule reference merge, print-format list, dummy message template, validate-payment guard, payment order mapping, open-payment-request query planning, existing Payment Entry/Payment Request amount/Integration Request query planning, old-payment-request cancellation planning, available schedule filtering, and discard/cancel/paid lifecycle status plans in `accounts_payment_request`. Remaining live gateway/Frappe DB orchestration is not yet parity-tested. |
