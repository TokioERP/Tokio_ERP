...
| `financial_report_template` | 8 | 6 | 1 | 1 | ported | Rust preserves financial report template registry names, modules, report types, row counts, all 29 account category seed rows, template test fixture rows, dependency resolver ordering/cycle detection, formula calculator arithmetic/math/error handling, and engine data models/filter validation helpers in `accounts_financial_report_template_registry` and `accounts_financial_report_engine`. Query builder, filter expression parser, data collector, template JSON, and DB/query/formatter/chart engine side effects remain external. |
