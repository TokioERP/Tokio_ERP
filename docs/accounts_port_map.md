...
| `pos_invoice_merge_log` | 5 | 3 | 1 | 1 | ported | Rust covers metadata, duplicate/customer/status validations, customer/accounting-dimension grouping, serial-return split grouping, consolidate/unconsolidate queue thresholds, enqueue scheduler/error helpers, merge invoice item/tax/payment/loyalty/rounding/dimension aggregation, and controller hooks in `accounts_pos_invoice_merge_log`. Submit/cancel DB side-effect planning remains to reach `parity_tested`. |
