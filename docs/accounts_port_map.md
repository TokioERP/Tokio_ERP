...
| `payment_request` | 7 | 4 | 1 | 2 | ported | Initial Rust batch covers allowed doctypes, metadata surface, payment reference validation, request amount JSON summing, submit status/outstanding planning, core get_amount branches, and Payment Entry Reference status/outstanding updates in `accounts_payment_request`. Remaining gateway/Frappe DB orchestration is not yet parity-tested. |
