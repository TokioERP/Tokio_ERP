...
| `dashboard_chart_source` | 2 | 5 | 3 | 1 | 1 | parity_tested | Rust covers Account Balance Timeline date bucketing, GL Entry query planning, account/chart validation errors, debit-credit/root-type balance rules, cumulative balance-sheet totals, and dashboard dataset shape in `accounts_account_balance_timeline`. JSON/JS chart-source metadata remains external. |
...
| `accounts/test/test_reports.py` | `tests/accounts_test_reports.rs` | parity_tested | Rust preserves the Accounts script-report execution matrix, default date/company filters, optional-filter shape, and filter payloads from ERPNext `test_reports.py`. Live Frappe report execution remains external integration. |
...
|---|---:|---:|---:|---:|---|---|
| `account` | 106 | 9 | 94 | 2 | parity_tested | Rust covers metadata, account validation, parent/root/type/currency/freeze/default-account guards, conversion plans, child-company account propagation plans, autoname/currency/search helpers, account-number update and merge planning, and ERPNext `test_account.py` regressions for account-number/name trimming, merge parent/group/root/currency guards, child-company sync and rename blocking/allowance, currency-change-after-ledger guard, and percent-bearing account balance query planning in `accounts_account`. JSON/JS/chart metadata kept external. |
...
| `bank_reconciliation_tool` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata/client hooks, bank transaction filters, account balance formula, bank transaction updates, JE/PE creation plans, auto-reconcile branching/messages, linked-payment matching source selection, allocation subtraction, and reconcile-vouchers transaction method order in `accounts_bank_reconciliation_tool`. JSON/JS kept external. |
| `bank_statement_import` | 7 | 3 | 1 | 2 | parity_tested | Rust covers metadata/client hooks, MT940 preprocessing and required-tag detection, template row filtering, bank-account column insertion including ERPNext's first-column append quirk, mapping/status aggregation, import-log query planning, and upload document initialization in `accounts_bank_statement_import`. JSON/JS kept external. |
| `bank_transaction` | 10 | 7 | 1 | 2 | parity_tested | Rust covers metadata, controller lifecycle hooks, allocation/unallocation/status math, duplicate-reference and currency guards, payment-entry add/remove/delink flows, clearance target planning including Sales Invoice child rows, linked Bank Transaction allocation, GL allocation grouping, remove-from-bank-transaction planning, included/excluded fee behavior, auto party matching order/fuzzy cutoff rules, and upload header/data mapping in `accounts_bank_transaction`, `accounts_bank_transaction_auto_match_party`, and `accounts_bank_transaction_upload`. JSON/JS kept external. |
...
| `budget` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata including cumulative-expense control fields, fiscal-year/company and duplicate/account validations, applicable flag/nulling behavior, existing-expense guard, budget distribution generation/recalculation/total validation including monthly equal distribution, budget record orchestration, annual/monthly action selection, expense comparison messages/breakup, MR/PO/requested/ordered/actual query planning, item default fallback, accumulated monthly budget helper, revise-budget planning, and ERPNext `test_budget.py` regression branches in `accounts_budget`. JSON/JS UI metadata remains external. |
...
| `chart_of_accounts_importer` | 6 | 3 | 1 | 1 | parity_tested | Rust covers metadata, validate hook, file extension guard, company/GL-entry checks, template column validation, CSV/Excel row normalization, nested COA forest building and parent/root errors, root/report/mandatory account helpers, template rows, unset-existing-data planning, default-account reset planning, and pass/no-op Python test parity in `accounts_chart_of_accounts_importer`. CSV/JSON/JS kept external. |
| `cheque_print_template` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata, cheque positioning defaults, max attachment/sort/autoname flags, print-format creation/update planning, Jinja HTML generation including A4 top offset and fallback account-pay message, has-print-format side effect planning, and pass/no-op Python test parity in `accounts_cheque_print_template`. JSON/JS kept external. |
...
| `financial_report_template` | 8 | 6 | 1 | 1 | parity_tested | Rust preserves financial report template registry names, modules, report types, row counts, all 29 account category seed rows, template test fixture rows, dependency resolver ordering/cycle detection, formula calculator arithmetic/math/error handling, filter expression parser condition planning/invalid-shape rejection, formula field extract/update helpers, row processing for account/API/formula/layout rows, query builder running balance/account-meta/accumulation helpers, detail row building, chart generation, growth view transformation, and engine data models/filter validation helpers in `accounts_financial_report_template_registry` and `accounts_financial_report_engine`. Query builder DB fetching, data collector, template JSON, and DB/query/formatter engine side effects remain external. |
...
| `gl_entry` | 6 | 3 | 1 | 1 | parity_tested | Rust covers metadata/autoname, mandatory/account/cost-center/party/currency/reporting-currency/fiscal-year validations, P&L cost-center and opening-entry guards, PL/BS accounting dimension checks, balance-type and frozen-account validations, outstanding amount planning for SI/PI/JE/Fee branches, against-account updates, cancel guard, index creation, temporary GL/SLE rename planning, and ERPNext `test_gl_entry.py` regressions for JE round-off GL rows, temporary name/series updates, non-receivable/payable party-type rejection, and Equity/Shareholder allowance in `accounts_gl_entry`. JSON/JS/README remain external. |
...
| `journal_entry` | 8 | 3 | 2 | 2 | parity_tested | Rust covers metadata, controller hooks, opening/clearance defaults, party validation, advance direction guards, multi-currency/exchange-rate handling, company-currency amount calculation, debit/credit totals, reference validation for SO/PO/SI/PI, against-account assignment, remarks/print totals, get-balance balancing row, GL map rows, payment-entry draft helpers, outstanding/account-detail helpers, reverse/inter-company journal-entry draft mapping, and ERPNext `test_journal_entry.py` regressions for stock-account perpetual-inventory guard and customer credit-limit checks in `accounts_journal_entry`. Lifecycle DB writes, asset/depreciation, invoice discounting, reposting, inter-company persistence, and live Frappe query side effects remain external integration. |
...
| `payment_entry` | 6 | 4 | 1 | 1 | parity_tested | Rust covers metadata, controller hooks, party account setup, internal-transfer clearing, payment type/mandatory/duplicate reference/bank reference guards, receive negative-outstanding guard, allocated amount guard for non-customer/supplier parties, blocked supplier payment guards with release-date parity, on-hold Purchase Invoice reference rejection, received/base amount calculation, allocated/unallocated/difference amount calculation, exchange gain/loss deduction row maintenance, title/remarks/status/in-words helpers, transaction currency/rate selection including same-currency defaults, advance-reference gain/loss bypass, and party/bank/deduction GL map planning in `accounts_payment_entry`. Live Frappe DB query branches for latest outstanding/payment-term allocation, taxes, payment requests, payment schedule updates, exchange-rate lookup, advance-as-liability, reconciliation, and exchange gain/loss journal posting remain integration-owned outside the pure Rust parity surface. |
...
| `payment_ledger_entry` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata, account/company/type validation, ledger active/group/company checks, accounting-dimension allow/restrict and mandatory PL/BS checks, on-update validation/outstanding-update gating, doctype index planning, and deterministic ERPNext payment-ledger expected rows in `accounts_payment_ledger_entry`. Actual payment/invoice/JV persistence remains external integration. JSON kept external. |
...
| `payment_reconciliation` | 5 | 3 | 1 | 1 | parity_tested | Rust covers virtual controller defaults/no-op persistence hooks, payment/JV/return invoice/dr-cr note/outstanding invoice query plans, allocation and exchange-difference logic, validation, reconciliation route planning, tax allocation no-op, and dimension filter helpers in `accounts_payment_reconciliation`. JSON/JS and live Frappe DB mutation remain external integration. |
...
| `payment_request` | 7 | 4 | 1 | 2 | parity_tested | Rust covers allowed doctypes, metadata surface, validate/on-submit/before-submit/cancel/discard/paid lifecycle planning, payment reference validation, payment-request amount/currency/subscription validation, request amount JSON summing, phone payment payloads, gateway validation/lookup plans, payment URL/email/communication plans, core get_amount branches, create-payment-entry planning, Payment Entry Reference payment-request allocation/splitting, Payment Entry Reference status/outstanding updates, schedule reference merge, print-format list, dummy message template, validate-payment guard, payment order mapping, make-payment-request orchestration, open-payment-request query planning, existing Payment Entry/Payment Request amount/Integration Request query planning, old-payment-request cancellation planning, subscription details lookup, existing references lookup, and available schedule filtering in `accounts_payment_request`. Live Frappe DB writes and external gateway calls remain integration-owned. |
...
| `period_closing_voucher` | 6 | 3 | 1 | 1 | parity_tested | Rust covers metadata, validation guards, lifecycle submit/cancel branches, GL closing entry construction, balance-sheet closing logic, processing/cancellation plans, account closing balance filters, fiscal period helpers, and ERPNext source test scenarios in `accounts_period_closing_voucher`. JSON/JS and live Frappe persistence remain external integration. |
...
| `pos_invoice` | 7 | 4 | 1 | 2 | parity_tested | Rust covers metadata, basic POS/customer validation, mode-of-payment guard, payment amount sign rules, change amount/account validation, POS profile company guard, outstanding amount, full status transitions including discounted and credit-note branches, before-submit/before-cancel/on-cancel planning, loyalty transaction/on-submit/on-cancel orchestration, phone payment completion and request payloads, stock availability helpers, product-bundle availability, reserved quantity aggregation, stock insufficiency messages, serial/batch required-field guards, return item quantity/serial guards, POS profile field/default and missing-value planning, reset mode-of-payment planning, update-payments totals, sales-return mapping, merge-log planning, item query filtering handoff, Sales Invoice payment row creation, unallocated payment cleanup, return Sales Invoice creation/consolidation planning, POS invoice merge regression totals for discount, inclusive-tax discount, and selling-price validation scenarios, and serial/batch bundle delink-submit planning in `accounts_pos_invoice`/`accounts_pos_invoice_merge`. Live Frappe DB writes and persistence remain external integration. |
...
| `pos_invoice_merge_log` | 5 | 3 | 1 | 1 | parity_tested | Rust covers metadata, duplicate/customer/status validations, customer/accounting-dimension grouping, serial-return split grouping, sales-invoice defaults, return credit-note grouping, sales-invoice/credit-note merge save-submit reference planning, on-submit orchestration planning, consolidate/unconsolidate queue thresholds, enqueue scheduler/error helpers, merge invoice item/tax/payment/loyalty/rounding/dimension aggregation, submit/cancel POS invoice update, linked invoice/serial bundle side-effect planning, create/cancel merge-log success-path planning, and ERPNext test scenarios for consolidated invoices, credit notes, item-wise taxes, rounding, serial returns, accounting dimensions, and company propagation in `accounts_pos_invoice_merge_log`. JSON/JS kept external. |
...
| `pricing_rule` | 6 | 5 | 1 | 0 | parity_tested | Rust covers pricing rule metadata, validation, ERPNext cleanup semantics for `apply_on`, `applicable_for`, `rate_or_discount`, and other-item fields, discount application/removal plans including stacked percentage/amount discounts, transaction argument updates, UOM/detail helpers, filtering/priority/condition/qty-amount utilities, applied-rule parsing, free-item/product discount planning, and coupon validation/count planning in `accounts_pricing_rule` and `accounts_pricing_rule_utils`. JSON/JS kept external. |
...
| `purchase_invoice` | 9 | 5 | 2 | 1 | parity_tested | Rust covers controller metadata, status updater, onload/before-save hold behavior, release-date/cash/credit-to guards, supplier master hold guards for invoices, remarks, received percentage, hold actions, and core status transitions including ERPNext cancel precedence in `accounts_purchase_invoice`. DB-heavy ledger, tax, stock, previous-doc, and submit/cancel side effects remain external. |
...
| `sales_invoice` | 10 | 5 | 2 | 2 | parity_tested | Rust covers controller metadata, status updater, onload TDS plan, portal indicator, payment/base amount rollup, customer-order remarks, POS return/write-off/payment sign guards, subcontracted stock flag, and core status transitions including discounting suffix and ERPNext cancel precedence in `accounts_sales_invoice`. DB-heavy ledger, tax, stock, previous-doc, loyalty, timesheet, and submit/cancel side effects remain external. |
...
| `subscription` | 6 | 3 | 1 | 2 | parity_tested | Rust covers metadata including naming/row flags and set-once fields, period helpers, billing cycle checks, trial/end/calendar validations, party currency guard, status/grace rules, invoice generation gates/plans, item/prorate/deferred/dimension mapping, query plan shapes, cancel/restart/force-fetch/process behavior, and controller hooks in `accounts_subscription`. DB save/commit scheduler side effects remain external. |
| `subscription_invoice` | 5 | 3 | 1 | 1 | parity_tested | Python controller is pass/no-op; Rust covers child-table metadata, dynamic-link fields, index/search/sort flags, and controller behavior in `accounts_subscription_invoice`. JSON/test kept external. |
...
| `tax_withholding_entry` | 4 | 3 | 1 | 0 | parity_tested | Rust covers metadata, status/link difference helpers, withholding amount calculation, adjustment validations, update value helpers, clear-old-reference cancellation planning, return-invoice cancellation split/reindex planning, and controller hooks in `accounts_tax_withholding_entry`. Full DB-backed submission flow remains external integration. |
...
## Doctype Detail: `payment_ledger_entry`

Source: `../erpnext/apps/erpnext/erpnext/accounts/doctype/payment_ledger_entry`
Target: `src/erpnext/accounts/doctype/payment_ledger_entry`

### Behavior

- Python controller inherits `frappe.model.document.Document`.
- Rust covers `validate_account`, `validate_account_details`, `validate_allowed_dimensions`, `validate_dimensions_for_pl_and_bs`, and `on_update` decision planning.
- Rust mirrors `on_update` branches for `from_repost`, `delinked`, `adv_adj`, outstanding doctypes, update-outstanding flag, and reverse-depreciation skip.
- Rust covers `on_doctype_update` index planning for `against_voucher_no/against_voucher_type` and `voucher_no/voucher_type`.
- ERPNext payment-ledger tests are represented by deterministic expected row helpers for invoice/payment and invoice/Journal Entry rows, including partial payments and credit/debit note sign handling.
- Sales Invoice, Payment Entry, Journal Entry, GL Entry, and Payment Ledger Entry database persistence remain external integration boundaries.
- DocType metadata:
  ...
  - `sort_field`: `creation`
  - `sort_order`: `DESC`
  - `field_order`: `posting_date`, `company`, `account_type`, `account`, `party_type`, `party`, `due_date`, `voucher_detail_no`, `cost_center`, `finance_book`, `voucher_type`, `voucher_no`, `against_voucher_type`, `against_voucher_no`, `amount`, `account_currency`, `amount_in_account_currency`, `delinked`, `remarks`
...
|---|---|---|---|
| `__init__.py` | `mod.rs` | parity_tested | Python package marker represented by Rust module declarations. |
| `payment_ledger_entry.py` | `payment_ledger_entry.rs` | parity_tested | Validation, on-update gating, index planning, expected ledger row shaping, metadata, and hooks represented in Rust. |
| `test_payment_ledger_entry.py` | `tests/accounts_payment_ledger_entry.rs` | parity_tested | Rust covers deterministic controller behavior and ERPNext expected payment-ledger row shapes; transaction persistence remains external integration. |
| `payment_ledger_entry.json` | ERPNext metadata retained | external_kept | Runtime DocType schema remains owned by ERPNext/Frappe. Rust mirrors behavior-relevant metadata constants. |
