...
| `pos_invoice` | 7 | 4 | 1 | 2 | ported | Initial Rust batch covers metadata, basic POS/customer validation, mode-of-payment guard, payment amount sign rules, change amount/account validation, POS profile company guard, outstanding amount, status transitions, before-submit/before-cancel/on-cancel planning, and phone payment completion check in `accounts_pos_invoice`. Remaining POS stock, serial/batch, profile, payment request, return, merge, and item query orchestration is not yet parity-tested. |
